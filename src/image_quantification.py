...
# from discco import ImageQuant, straighten, interp_roi, offset_coordinates, view_stack, \
#     view_stack_jupyter, plot_quantification_jupyter, plot_quantification, plot_segmentation, plot_fits, \
#     plot_fits_jupyter, plot_segmentation_jupyter, in_notebook, erf, rolling_ave_2d, interp_2d_array
from discco import *
...
class Discco2:
    def __init__(self, img, roi, cytbg=None, membg=None, thickness=50, batch_norm=False, zerocap=True, pooling_rate=1,
        ...
        self.padded_size = int(max([np.ceil(r.shape[0] / pooling_rate) for r in self.roi]))
    ...
    def segment(self, freedom=10, roi_knots=20, lr=0.01, descent_steps=500):
        ...
        self.roi[:] = [interp_roi(offset_coordinates(roi, offsets_full), periodic=True) for roi, offsets_full in
                       zip(iq.roi, iq.offsets_full)]
    ...
    def quantify(self, lr=0.005, descent_steps=600, save_interim=False):
        ...
        # Gradient descent
        res_interim = self.gradient_descent_quantification(lr=lr, descent_steps=descent_steps,
    ...
    def calibrate_cytoplasm(self, lr=0.005, descent_steps=600, save_interim=False):
        ...
        # Cytbg calibration
        res_interim = self.gradient_descent_cytoplasm_calibration(lr=lr, descent_steps=descent_steps,
                                                                  save_interim=save_interim)
    ...
    def calibrate_membrane(self, lr=0.005, descent_steps=600, save_interim=False):
        ...
        # Cytbg calibration
        res_interim = self.gradient_descent_membrane_calibration(lr=lr, descent_steps=descent_steps,
                                                                 save_interim=save_interim)
    ...
    def gradient_descent_quantification(self, lr, descent_steps, save_interim):
        ...
        params = {'cyts_opt': self.cyts_opt, 'mems_opt': self.mems_opt}  # Create params list
    ...
    def gradient_descent_membrane_calibration(self, lr, descent_steps, save_interim):
        ...
        # Initialisation
        opt = optax.adam(learning_rate=lr)  # Set up optimiser
        params = {'cyts_opt': self.cyts_opt, 'mems_opt': self.mems_opt,
                  'membg_opt': self.membg_opt}  # Create params list
        opt_state = opt.init(params)  # Set up optimiser initial state
        self.losses = np.zeros([self.nimgs, descent_steps])  # Create empty losses array
    ...
    def gradient_descent_cytoplasm_calibration(self, lr, descent_steps, save_interim):
        # Initialisation
        opt = optax.adam(learning_rate=lr)  # Set up optimiser
        params = {'cyts_opt': self.cyts_opt, 'cytbg_opt': self.cytbg_opt}  # Create params list
        opt_state = opt.init(params)  # Set up optimiser initial state
        self.losses = np.zeros([self.nimgs, descent_steps])  # Create empty losses array
    ...
    def _preprocess_single(self, frame, roi):
        ...
        # Straighten
        straight = straighten(frame, roi, thickness=self.thickness, interp='cubic', periodic=True)
        ...
        if self.downsampling_rate != 1:
            ...
            if remainder != 0:
                pad = np.repeat(np.expand_dims(np.mean(straight[:, -remainder:], axis=1), -1),
                                (self.downsampling_rate - remainder), axis=1)
    ...
    def _preprocess_batch(self):
        ...
        target, norms, masks = zip(
            *[self._preprocess_single(frame, roi) for frame, roi in zip(self.img, self.roi)])
    ...
    def _init_params(self):
        ...
        if self.cytbg is not None:
            ...
            self.cytbg_opt = (1 + erf((np.arange(self.thickness) - self.thickness / 2) / self.sigma)) / 2
        ...
        if self.membg is not None:
            ...
            self.membg_opt = np.exp(
                -((np.arange(self.thickness) - self.thickness / 2) ** 2) / (2 * self.sigma ** 2))
    ...
    def compile_res(self, ids=None, extra_columns=None):
        ...
        if extra_columns is not None:
            df = df.reindex(
                columns=['EmbryoID', 'Position', 'Membrane signal', 'Cytoplasmic signal'] + list(extra_columns.keys()))
        else:
            df = df.reindex(columns=['EmbryoID', 'Position', 'Membrane signal', 'Cytoplasmic signal'])
    ...
    def plot_fits(self):
        ...
        if not jupyter:
            if self.stack:
                ...
                fig, ax = plot_fits(self.straight_images[0], self.straight_images_sim[0])
        else:
            if self.stack:
                fig, ax = plot_fits_jupyter(self.straight_images, self.straight_images_sim)
            else:
                fig, ax = plot_fits_jupyter(self.straight_images[0], self.straight_images_sim[0])
        return fig, ax
...
def masked_loss_function(sim, target, masks):
    ...
    mse = jnp.sum(sq_errors * jnp.expand_dims(masks, 1), axis=[1, 2]) / jnp.sum(masks, axis=1)  # masked average
    return mse
